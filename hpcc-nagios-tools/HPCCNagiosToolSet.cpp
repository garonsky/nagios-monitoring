...
bool CHPCCNagiosToolSet::generateNagiosEspServiceConfig(StringBuffer &strServiceConfig, const char* pEnvXML, const char* pConfigGenPath)
{
    ...
    while (pch != NULL)
    {
        ...
        else if (nCount % 7 == 6) // protocol
        {
            ...
            {
                strServiceConfig.append(P_NAGIOS_SEPERATOR).append(m_pUserMacro).append(P_NAGIOS_SEPERATOR).append(m_pPasswordMacro);
            }

            strServiceConfig.append(P_NAGIOS_SERVICE_END_BRACKET);
    ...
    while (pch != NULL)
    {
        ...
        else if (nCount % nNumValues == 4)
        {
            ...
            if (CHPCCNagiosToolSet::m_bCheckUsers == true)
            {
                ...
                if (CHPCCNagiosToolSet::m_bUseNPRE == false)
                {
                    strServiceConfig.append(P_NAGIOS_SEPERATOR).append(m_nUserNumberWarning);
                }
